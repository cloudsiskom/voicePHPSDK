...
```php
$iVRController = $client->getIVRController();
```
...
```php
function iVRDelete(IVRDeleteRequest $body): void
```
...
```php
function iVRDeleteItem(IVRDeleteItemRequest $body): void
```
...
```php
$body_ivrItemId = '';
$body = new Models\IVRDeleteItemRequest(
    $body_ivrItemId
);

$iVRController->iVRDeleteItem($body);
```
